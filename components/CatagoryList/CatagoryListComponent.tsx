...
import { getCatagoryItems } from "../../services/api.service";
...
const CatagoryListComponent = () => {
    ...
    const [catagoryItemsData, setcatagoryItemsData]: any = useState([]);
    console.log('data', catagoryItemsData);

    useEffect(() => {
        catagoryItems();
    }, [id]);

    const catagoryItems = async () => {

        const data = await getCatagoryItems(id);
        if (data)
            console.log('siam',data.data);
        setcatagoryItemsData(data.data);
    ...
    return (
        <>
            { catagoryItemsData ? <div className={`mx-2 py-3 ${styles.catagoryPage}`} >

                <nav className={`navbar py-0 fixed-top ${styles.topNav}`} >
                    <div className={styles.backBtn}>

                        {/* {catagoryItemsData.map((catagoryName: any) => ( */}
                            <h4 onClick={() => Router.back()}><i className="bi bi-arrow-left mx-3"></i>{catagoryItemsData.name}</h4>
                        {/* ))} */}
                ...
                <div className="row g-4">
                    ...
                    { catagoryItemsData?.map((catagoryList: any) => (
                            ...
                            <Link href={`../audiobook_details/${catagoryList?.id}`}>
                                ...
                                <div className={`d-flex justify-content-center align-items-center ${styles.catagoryCard}`} >
                                    ...
                                    <div className="py-2 mx-2">
                                        ...
                                        <p className={`mb-0 ${styles.textColor}`} ><small>{catagoryList.description.slice(0,200)}</small></p>
                                    </div>
                                </div>
                            </Link>
                        </div>
